        ...
        {isLoading ? (
          ...
          <>
              ...
              <div className="panel content-panel">
                ...
                {data.paths.length ? <div className="admin-list">{data.paths.map(path => (
                  <article className="admin-item" key={path.id}>
                    ...
                    <div className="row-actions"><Button variant="ghost" size="sm" className="copy-link-button" aria-label="نسخ رابط المسار" title="نسخ رابط المسار لإرساله للطالبات" onClick={() => { void navigator.clipboard.writeText(`${window.location.origin}/paths/${path.id}`); toast.success("تم نسخ رابط المسار — ابعتيه للطالبات مباشرة."); }}><Link2 className="size-4" /></Button><Button variant="outline" size="sm" onClick={() => publishPath.mutate({ pathId: path.id, isPublished: !path.isPublished })}>{path.isPublished ? "إخفاء" : "نشر"}</Button><DeleteButton title="حذف المسار" description={`سيتم حذف «${path.title}» نهائياً مع كل دروسه وفيديوهاته واختباره وسجلات تقدم الطالبات فيه. لا يمكن التراجع عن هذا الإجراء.`} pending={isPending("path", path.id)} onConfirm={() => deletePath.mutate({ pathId: path.id }, settleDelete)} /></div>
                  </article>
            ...
            <section className="panel assessment-library">
              ...
              {data.assessments.length ? <div className="lesson-admin-table">{data.assessments.slice(0, 8).map(assessment => <div className="lesson-admin-row" key={assessment.id}><Link2 /><div><b>{assessment.title}</b><p>{data.paths.find(path => path.id === assessment.pathId)?.title ?? "مسار غير محدد"} · الدرجة الكلية {assessment.maxScore}</p></div><Badge variant={assessment.isPublished ? "default" : "secondary"}>{assessment.isPublished ? "منشور" : "مسودة"}</Badge><Button variant="outline" size="sm" onClick={() => publishAssessment.mutate({ assessmentId: assessment.id, isPublished: !assessment.isPublished }, { onSuccess: () => toast.success(assessment.isPublished ? "تم إخفاء الاختبار." : "تم نشر الاختبار وأُرسل تنبيه للطالبات المسجلات في المسار.") })}>{assessment.isPublished ? "إخفاء" : "نشر"}</Button><DeleteButton title="حذف الاختبار" description={`سيتم حذف الاختبار «${assessment.title}» نهائياً مع كل النتائج المرتبطة به. لا يمكن التراجع عن هذا الإجراء.`} pending={isPending("assessment", assessment.id)} onConfirm={() => deleteAssessment.mutate({ assessmentId: assessment.id }, settleDelete)} /></div>)}</div> : <AdminEmpty icon={Link2} title="لم تتم إضافة اختبارات" text="أضيفي رابط اختبار واربطِيه بالمسار، ثم انشريه للطالبات." />}
            </section>
          </>
...
function CreateLessonDialog({ paths, onDone }: { paths: PathChoice[]; onDone: () => void }) {
  ...
  const [form, setForm] = useState({ pathId: 0, title: "", summary: "", lessonType: "article" as "article" | "workshop" | "resource", attachmentUrl: "", attachmentName: "", externalAttachmentUrl: "", durationMinutes: 20, position: 1 });
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    const external = form.externalAttachmentUrl.trim();
    const finalAttachmentUrl = form.attachmentUrl || external;
    const { externalAttachmentUrl: _omit, ...rest } = form;
    create.mutate({ ...rest, sourceUrl: undefined, attachmentUrl: finalAttachmentUrl || undefined, attachmentName: form.attachmentName || (finalAttachmentUrl && !form.attachmentUrl ? "PDF خارجي" : undefined) });
  };
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" className="quick-button" disabled={!paths.length}><FilePlus2 /> إضافة درس</Button></DialogTrigger><DialogContent dir="rtl"><DialogHeader><DialogTitle>إضافة درس وملف PDF</DialogTitle><DialogDescription>ارفعي ملف PDF مباشرةً من جهازك؛ يُربط بالدرس تلقائياً ولا يلزم نسخ أي رابط. أو الصقي رابط PDF خارجياً (مثل Google Drive) — الخيار ده مفيد مؤقتاً لما يكون مخزن Vercel موقوفاً، لكن اتركي الملف المرفوع هو الأولي عند توفر الاثنين.</DialogDescription></DialogHeader><form className="form-stack" onSubmit={submit}><Field label="المسار"><PathSelect paths={paths} value={form.pathId} onChange={pathId => setForm({ ...form, pathId })} /></Field><Field label="عنوان الدرس"><Input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field><Field label="ملخص قصير"><Textarea value={form.summary} onChange={e => setForm({ ...form, summary: e.target.value })} /></Field><DirectFileUpload mode="document" currentFileName={form.attachmentName} onUploaded={({ url, name }) => setForm({ ...form, attachmentUrl: url, attachmentName: name })} /><Field label="رابط PDF خارجي (اختياري — يُستخدم لو ما رفعتِ ملفاً)"><Input type="url" dir="ltr" value={form.externalAttachmentUrl} onChange={e => setForm({ ...form, externalAttachmentUrl: e.target.value })} placeholder="https://drive.google.com/file/d/..." /></Field><div className="form-row"><Field label="نوع المحتوى"><Select value={form.lessonType} onValueChange={(lessonType: "article" | "workshop" | "resource") => setForm({ ...form, lessonType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="article">مقال</SelectItem><SelectItem value="workshop">ورشة</SelectItem><SelectItem value="resource">مورد</SelectItem></SelectContent></Select></Field><Field label="المدة بالدقائق"><Input type="number" min="1" value={form.durationMinutes} onChange={e => setForm({ ...form, durationMinutes: Number(e.target.value) })} /></Field></div><Button className="academy-button" type="submit" disabled={create.isPending || !form.pathId || (!form.attachmentUrl && !form.externalAttachmentUrl.trim())}>{create.isPending ? "يتم الحفظ..." : "حفظ ونشر الدرس"}</Button></form></DialogContent></Dialog>;
}
...
function CreateVideoDialog({ paths, onDone }: { paths: PathChoice[]; onDone: () => void }) {
  ...
  const create = trpc.admin.createLesson.useMutation({ onSuccess: () => { toast.success("تمت إضافة الفيديو ونشره للطالبات، وأُرسل تنبيه للطالبات المسجلات في المسار."); setOpen(false); onDone(); }, onError: error => toast.error(error.message) });
