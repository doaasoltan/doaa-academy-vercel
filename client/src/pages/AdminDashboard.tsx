...
import { BarChart3, BookOpen, CheckCircle2, ClipboardCheck, FilePlus2, FileText, GraduationCap, LayoutDashboard, Link2, Pencil, PlayCircle, Plus, Send, Sparkles, Trash2, Upload, Users } from "lucide-react";
import { useEffect, useState } from "react";
import { useLocation } from "wouter";

const menu = [
  { icon: LayoutDashboard, label: "لوحة الإدارة", path: "/admin", sectionId: "admin-overview" },
  { icon: BookOpen, label: "المسارات والدروس", path: "/admin", sectionId: "admin-paths" },
  { icon: ClipboardCheck, label: "الاختبارات والنتائج", path: "/admin", sectionId: "admin-results" },
  { icon: Users, label: "متابعة الطلاب", path: "/admin", sectionId: "admin-students" },
];
...
type LessonItem = { id: number; pathId: number; title: string; summary: string | null | undefined; lessonType: string; sourceUrl: string | null | undefined; attachmentUrl: string | null | undefined; attachmentName: string | null | undefined; durationMinutes: number; position: number; isPublished: boolean };
type AssessmentItem = { id: number; pathId: number; title: string; description: string | null; externalUrl: string; maxScore: number; position: number; isPublished: boolean };
...
export default function AdminDashboard() {
  ...
  const deleteLesson = trpc.admin.deleteLesson.useMutation({ onSuccess: () => { toast.success("تم حذف المحتوى نهائياً."); refresh(); }, onError: error => toast.error(error.message) });
  const deleteAssessment = trpc.admin.deleteAssessment.useMutation({ onSuccess: () => { toast.success("تم حذف الاختبار نهائياً."); refresh(); }, onError: error => toast.error(error.message) });
    ...
    <DashboardLayout menuItems={menu} title="لوحة المعلمة">
      <div className="dashboard-page">
        ...
        {isLoading ? (
          ...
          <>
            <section className="stat-grid" id="admin-overview">
            ...
            <section className="admin-main-grid bottom-grid">
              <div className="panel results-panel" id="admin-results">
            ...
            <section className="panel lesson-library" id="admin-lessons">
              ...
              {data.lessons.filter(lesson => lesson.lessonType !== "video").length ? <div className="lesson-admin-table">{data.lessons.filter(lesson => lesson.lessonType !== "video").slice(0, 8).map(lesson => <div className="lesson-admin-row" key={lesson.id}><FileText /><div><b>{lesson.title}</b><p>{data.paths.find(path => path.id === lesson.pathId)?.title ?? "مسار غير محدد"} · {lesson.durationMinutes} دقيقة{lesson.attachmentUrl && lesson.attachmentUrl.startsWith("http") ? " · رابط PDF خارجي" : ""}</p>{lesson.attachmentUrl && <a className="file-open-link" href={lesson.attachmentUrl} target="_blank" rel="noreferrer"><FileText className="size-3" /> فتح PDF</a>}</div><Badge variant={lesson.isPublished ? "default" : "secondary"}>{lesson.isPublished ? "منشور" : "مسودة"}</Badge><div className="row-actions"><EditLessonDialog lesson={lesson} paths={data.paths} onDone={refresh} /><DeleteContentButton label="حذف الدرس" message={`سيتم حذف الدرس "${lesson.title}" نهائياً ولا يمكن التراجع عن ذلك.`} pending={deleteLesson.isPending} onConfirm={() => deleteLesson.mutate({ lessonId: lesson.id })} /><Button variant="outline" size="sm" onClick={() => publishLesson.mutate({ lessonId: lesson.id, isPublished: !lesson.isPublished })}>{lesson.isPublished ? "إخفاء" : "نشر"}</Button></div></div>)}</div> : <AdminEmpty icon={FilePlus2} title="مكتبة الدروس فارغة" text="أضيفي درساً وارفعِي ملف PDF من جهازك لربطه بالمسار." />}
            </section>

            <section className="panel video-library" id="admin-videos">
            ...
            <section className="panel assessment-library" id="admin-assessments">
              ...
              {data.assessments.length ? <div className="lesson-admin-table">{data.assessments.slice(0, 8).map(assessment => <div className="lesson-admin-row" key={assessment.id}><Link2 /><div><b>{assessment.title}</b><p>{data.paths.find(path => path.id === assessment.pathId)?.title ?? "مسار غير محدد"} · الدرجة الكلية {assessment.maxScore}</p></div><Badge variant={assessment.isPublished ? "default" : "secondary"}>{assessment.isPublished ? "منشور" : "مسودة"}</Badge><div className="row-actions"><EditAssessmentDialog assessment={assessment} paths={data.paths} onDone={refresh} /><DeleteContentButton label="حذف الاختبار" message={`سيتم حذف الاختبار "${assessment.title}" وجميع نتائجه نهائياً ولا يمكن التراجع عن ذلك.`} pending={deleteAssessment.isPending} onConfirm={() => deleteAssessment.mutate({ assessmentId: assessment.id })} /><Button variant="outline" size="sm" onClick={() => publishAssessment.mutate({ assessmentId: assessment.id, isPublished: !assessment.isPublished })}>{assessment.isPublished ? "إخفاء" : "نشر"}</Button></div></div>)}</div> : <AdminEmpty icon={Link2} title="لم تتم إضافة اختبارات" text="أضيفي رابط اختبار واربطِيه بالمسار، ثم انشريه للطالبات." />}
            </section>
          </>
...
function CreateLessonDialog({ paths, onDone }: { paths: PathChoice[]; onDone: () => void }) {
  ...
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    const externalPdfUrl = form.externalPdfUrl.trim();
    if (externalPdfUrl && !/^https?:\/\/.+\..+/.test(externalPdfUrl)) { toast.error("رابط ملف PDF الخارجي غير صالح. الصقي رابط https صحيح."); return; }
    const attachmentUrl = form.attachmentUrl || externalPdfUrl || undefined;
    const attachmentName = form.attachmentUrl ? form.attachmentName : (externalPdfUrl ? "رابط PDF خارجي" : undefined);
    if (!attachmentUrl) { toast.error("ارفعي ملف PDF من جهازك أو أضيفي رابطاً خارجياً."); return; }
    create.mutate({ pathId: form.pathId, title: form.title, summary: form.summary, lessonType: form.lessonType, attachmentUrl, attachmentName, durationMinutes: form.durationMinutes, position: form.position });
  };
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" className="quick-button" disabled={!paths.length}><FilePlus2 /> إضافة درس</Button></DialogTrigger><DialogContent dir="rtl"><DialogHeader><DialogTitle>إضافة درس وملف PDF</DialogTitle><DialogDescription>ارفعي ملف PDF من جهازك، أو الصقي رابط ملف خارجي (مثل Google Drive) — عند توفر الاثنين معاً يُستخدم الملف المرفوع.</DialogDescription></DialogHeader><form className="form-stack" onSubmit={submit}><Field label="المسار"><PathSelect paths={paths} value={form.pathId} onChange={pathId => setForm({ ...form, pathId })} /></Field><Field label="عنوان الدرس"><Input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field><Field label="ملخص قصير"><Textarea value={form.summary} onChange={e => setForm({ ...form, summary: e.target.value })} /></Field><DirectFileUpload mode="document" currentFileName={form.attachmentName} onUploaded={({ url, name }) => setForm({ ...form, attachmentUrl: url, attachmentName: name })} /><Field label="رابط PDF خارجي (اختياري)"><Input type="url" dir="ltr" value={form.externalPdfUrl} onChange={e => setForm({ ...form, externalPdfUrl: e.target.value })} placeholder="https://drive.google.com/..." /></Field><div className="form-row"><Field label="نوع المحتوى"><Select value={form.lessonType} onValueChange={(lessonType: "article" | "workshop" | "resource") => setForm({ ...form, lessonType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="article">مقال</SelectItem><SelectItem value="workshop">ورشة</SelectItem><SelectItem value="resource">مورد</SelectItem></SelectContent></Select></Field><Field label="المدة بالدقائق"><Input type="number" min="1" value={form.durationMinutes} onChange={e => setForm({ ...form, durationMinutes: Number(e.target.value) })} /></Field></div><Button className="academy-button" type="submit" disabled={create.isPending || !form.pathId || (!form.attachmentUrl && !form.externalPdfUrl.trim())}>{create.isPending ? "يتم الحفظ..." : "حفظ ونشر الدرس"}</Button></form></DialogContent></Dialog>;
}
...
function DeleteContentButton({ label, message, pending, onConfirm }: { label: string; message: string; pending: boolean; onConfirm: () => void }) {
  return <AlertDialog><AlertDialogTrigger asChild><Button variant="outline" size="sm" className="row-icon-button danger" title={label} aria-label={label}><Trash2 className="size-4" /></Button></AlertDialogTrigger><AlertDialogContent dir="rtl"><AlertDialogHeader><AlertDialogTitle>{label}</AlertDialogTitle><AlertDialogDescription>{message}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>تراجع</AlertDialogCancel><AlertDialogAction className="danger-action" disabled={pending} onClick={() => onConfirm()}>{pending ? "يتم الحذف..." : "تأكيد الحذف"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>;
}

function EditLessonDialog({ lesson, paths, onDone }: { lesson: LessonItem; paths: PathChoice[]; onDone: () => void }) {
  const [open, setOpen] = useState(false);
  const update = trpc.admin.updateLesson.useMutation({ onSuccess: () => { toast.success("تم حفظ تعديلات الدرس."); setOpen(false); onDone(); }, onError: error => toast.error(error.message) });
  const splitAttachment = (attachmentUrl: string | null | undefined, attachmentName: string | null | undefined) => {
    if (attachmentUrl && /^https?:\/\//.test(attachmentUrl)) return { uploaded: "", uploadedName: "", external: attachmentUrl };
    return { uploaded: attachmentUrl ?? "", uploadedName: attachmentName ?? "", external: "" };
  };
  const initial = splitAttachment(lesson.attachmentUrl, lesson.attachmentName);
  const lessonTypeOf = (value: string) => (value === "video" ? "article" : value) as "article" | "workshop" | "resource";
  const [form, setForm] = useState({ pathId: lesson.pathId, title: lesson.title, summary: lesson.summary ?? "", lessonType: lessonTypeOf(lesson.lessonType), attachmentUrl: initial.uploaded, attachmentName: initial.uploadedName, externalPdfUrl: initial.external, durationMinutes: lesson.durationMinutes, position: lesson.position });
  useEffect(() => {
    if (open) {
      const parts = splitAttachment(lesson.attachmentUrl, lesson.attachmentName);
      setForm({ pathId: lesson.pathId, title: lesson.title, summary: lesson.summary ?? "", lessonType: lessonTypeOf(lesson.lessonType), attachmentUrl: parts.uploaded, attachmentName: parts.uploadedName, externalPdfUrl: parts.external, durationMinutes: lesson.durationMinutes, position: lesson.position });
    }
  }, [open, lesson]);
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    ...
    const attachmentUrl = form.attachmentUrl || externalPdfUrl || undefined;
    const attachmentName = form.attachmentUrl ? form.attachmentName : (externalPdfUrl ? "رابط PDF خارجي" : undefined);
    update.mutate({ lessonId: lesson.id, pathId: form.pathId, title: form.title, summary: form.summary, lessonType: form.lessonType, attachmentUrl, attachmentName, durationMinutes: form.durationMinutes, position: form.position });
  };
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" size="sm" className="row-icon-button" title="تعديل الدرس" aria-label="تعديل الدرس"><Pencil className="size-4" /></Button></DialogTrigger><DialogContent dir="rtl"><DialogHeader><DialogTitle>تعديل الدرس</DialogTitle><DialogDescription>عدّلي بيانات الدرس، أو استبدلي ملف PDF بملف جديد من جهازك، أو حدّثي الرابط الخارجي.</DialogDescription></DialogHeader><form className="form-stack" onSubmit={submit}><Field label="المسار"><PathSelect paths={paths} value={form.pathId} onChange={pathId => setForm({ ...form, pathId })} /></Field><Field label="عنوان الدرس"><Input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field><Field label="ملخص قصير"><Textarea value={form.summary} onChange={e => setForm({ ...form, summary: e.target.value })} /></Field><DirectFileUpload mode="document" currentFileName={form.attachmentName || (form.attachmentUrl ? "ملف مرفوع حالياً" : "")} onUploaded={({ url, name }) => setForm({ ...form, attachmentUrl: url, attachmentName: name })} />{form.attachmentUrl && <Button type="button" variant="ghost" size="sm" onClick={() => setForm({ ...form, attachmentUrl: "", attachmentName: "" })}>إزالة الملف المرفوع والاكتفاء بالرابط</Button>}<Field label="رابط PDF خارجي"><Input type="url" dir="ltr" value={form.externalPdfUrl} onChange={e => setForm({ ...form, externalPdfUrl: e.target.value })} placeholder="https://drive.google.com/..." /></Field><div className="form-row"><Field label="نوع المحتوى"><Select value={form.lessonType} onValueChange={(lessonType: "article" | "workshop" | "resource") => setForm({ ...form, lessonType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="article">مقال</SelectItem><SelectItem value="workshop">ورشة</SelectItem><SelectItem value="resource">مورد</SelectItem></SelectContent></Select></Field><Field label="المدة بالدقائق"><Input type="number" min="1" value={form.durationMinutes} onChange={e => setForm({ ...form, durationMinutes: Number(e.target.value) })} /></Field></div><Button className="academy-button" type="submit" disabled={update.isPending || !form.pathId}>{update.isPending ? "يتم الحفظ..." : "حفظ التعديلات"}</Button></form></DialogContent></Dialog>;
}

function EditVideoDialog({ video, paths, onDone }: { video: LessonItem; paths: PathChoice[]; onDone: () => void }) {
  const [open, setOpen] = useState(false);
  const update = trpc.admin.updateLesson.useMutation({ onSuccess: () => { toast.success("تم حفظ تعديلات الفيديو."); setOpen(false); onDone(); }, onError: error => toast.error(error.message) });
  const splitSource = (sourceUrl: string | null | undefined) => {
    if (sourceUrl && /^https?:\/\//.test(sourceUrl)) return { uploaded: "", external: sourceUrl };
    return { uploaded: sourceUrl ?? "", external: "" };
  };
  const initial = splitSource(video.sourceUrl);
  const [form, setForm] = useState({ pathId: video.pathId, title: video.title, summary: video.summary ?? "", sourceUrl: initial.uploaded, externalUrl: initial.external, durationMinutes: video.durationMinutes, position: video.position });
  useEffect(() => {
    if (open) {
      const parts = splitSource(video.sourceUrl);
      setForm({ pathId: video.pathId, title: video.title, summary: video.summary ?? "", sourceUrl: parts.uploaded, externalUrl: parts.external, durationMinutes: video.durationMinutes, position: video.position });
    }
  }, [open, video]);
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    ...
    const sourceUrl = form.sourceUrl || externalUrl;
    if (!sourceUrl) { toast.error("ارفعي ملف الفيديو من جهازك أو أضيفي رابط يوتيوب."); return; }
    update.mutate({ lessonId: video.id, pathId: form.pathId, title: form.title, summary: form.summary, lessonType: "video", sourceUrl, durationMinutes: form.durationMinutes, position: form.position });
  };
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" size="sm" className="row-icon-button" title="تعديل الفيديو" aria-label="تعديل الفيديو"><Pencil className="size-4" /></Button></DialogTrigger><DialogContent dir="rtl"><DialogHeader><DialogTitle>تعديل الفيديو</DialogTitle><DialogDescription>عدّلي بيانات الفيديو، أو استبدلي الملف المرفوع، أو حدّثي رابط يوتيوب.</DialogDescription></DialogHeader><form className="form-stack" onSubmit={submit}><Field label="المسار"><PathSelect paths={paths} value={form.pathId} onChange={pathId => setForm({ ...form, pathId })} /></Field><Field label="عنوان الفيديو"><Input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field><Field label="وصف مختصر"><Textarea value={form.summary} onChange={e => setForm({ ...form, summary: e.target.value })} /></Field><DirectFileUpload mode="video" currentFileName={form.sourceUrl ? "ملف فيديو مرفوع حالياً" : ""} onUploaded={({ url }) => setForm({ ...form, sourceUrl: url })} />{form.sourceUrl && <Button type="button" variant="ghost" size="sm" onClick={() => setForm({ ...form, sourceUrl: "" })}>إزالة الملف المرفوع والاكتفاء بالرابط</Button>}<Field label="رابط يوتيوب أو فيديو خارجي"><Input type="url" dir="ltr" value={form.externalUrl} onChange={e => setForm({ ...form, externalUrl: e.target.value })} placeholder="https://www.youtube.com/watch?v=..." /></Field><Field label="مدة الفيديو بالدقائق"><Input required type="number" min="1" value={form.durationMinutes} onChange={e => setForm({ ...form, durationMinutes: Number(e.target.value) })} /></Field><Button className="academy-button" type="submit" disabled={update.isPending || !form.pathId || (!form.sourceUrl && !form.externalUrl.trim())}>{update.isPending ? "يتم الحفظ..." : "حفظ التعديلات"}</Button></form></DialogContent></Dialog>;
}

function EditAssessmentDialog({ assessment, paths, onDone }: { assessment: AssessmentItem; paths: PathChoice[]; onDone: () => void }) {
  const [open, setOpen] = useState(false);
  const update = trpc.admin.updateAssessment.useMutation({ onSuccess: () => { toast.success("تم حفظ تعديلات الاختبار."); setOpen(false); onDone(); }, onError: error => toast.error(error.message) });
  const [form, setForm] = useState({ pathId: assessment.pathId, title: assessment.title, description: assessment.description ?? "", externalUrl: assessment.externalUrl, maxScore: assessment.maxScore, position: assessment.position });
  useEffect(() => {
    if (open) setForm({ pathId: assessment.pathId, title: assessment.title, description: assessment.description ?? "", externalUrl: assessment.externalUrl, maxScore: assessment.maxScore, position: assessment.position });
  }, [open, assessment]);
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" size="sm" className="row-icon-button" title="تعديل الاختبار" aria-label="تعديل الاختبار"><Pencil className="size-4" /></Button></DialogTrigger><DialogContent dir="rtl"><DialogHeader><DialogTitle>تعديل الاختبار</DialogTitle><DialogDescription>عدّلي عنوان الاختبار أو رابطه أو درجته الكلية.</DialogDescription></DialogHeader><form className="form-stack" onSubmit={e => { e.preventDefault(); update.mutate({ assessmentId: assessment.id, ...form }); }}><Field label="المسار"><PathSelect paths={paths} value={form.pathId} onChange={pathId => setForm({ ...form, pathId })} /></Field><Field label="عنوان الاختبار"><Input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field><Field label="الرابط الخارجي"><Input required type="url" dir="ltr" value={form.externalUrl} onChange={e => setForm({ ...form, externalUrl: e.target.value })} placeholder="https://..." /></Field><Field label="وصف مختصر"><Textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></Field><Field label="الدرجة الكلية"><Input type="number" min="1" value={form.maxScore} onChange={e => setForm({ ...form, maxScore: Number(e.target.value) })} /></Field><Button className="academy-button" type="submit" disabled={update.isPending || !form.pathId}>{update.isPending ? "يتم الحفظ..." : "حفظ التعديلات"}</Button></form></DialogContent></Dialog>;
}
