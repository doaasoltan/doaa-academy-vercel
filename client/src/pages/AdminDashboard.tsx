...
function EditLessonDialog({ lesson, paths, onDone }: { lesson: LessonItem; paths: PathChoice[]; onDone: () => void }) {
  ...
  const [form, setForm] = useState({ pathId: lesson.pathId, title: lesson.title, summary: lesson.summary ?? "", lessonType: (lesson.lessonType === "video" ? "article" : lesson.lessonType) as "article" | "workshop" | "resource", attachmentUrl: lesson.attachmentUrl ?? "", attachmentName: lesson.attachmentName ?? "", durationMinutes: lesson.durationMinutes, position: lesson.position });
  useEffect(() => {
    if (open) setForm({ pathId: lesson.pathId, title: lesson.title, summary: lesson.summary ?? "", lessonType: (lesson.lessonType === "video" ? "article" : lesson.lessonType) as "article" | "workshop" | "resource", attachmentUrl: lesson.attachmentUrl ?? "", attachmentName: lesson.attachmentName ?? "", durationMinutes: lesson.durationMinutes, position: lesson.position });
  }, [open, lesson]);
  const submit = (event: React.FormEvent) => {
    event.preventDefault();
    update.mutate({ lessonId: lesson.id, pathId: form.pathId, title: form.title, summary: form.summary, lessonType: form.lessonType, attachmentUrl: form.attachmentUrl || undefined, attachmentName: form.attachmentName || undefined, durationMinutes: form.durationMinutes, position: form.position });
  };
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" size="sm" className="row-icon-button" title="تعديل الدرس" aria-label="تعديل الدرس"><Pencil className="size-4" /></Button></DialogTrigger><DialogContent dir="rtl"><DialogHeader><DialogTitle>تعديل الدرس</DialogTitle><DialogDescription>عدّلي بيانات الدرس أو استبدلي ملف PDF بملف جديد من جهازك.</DialogDescription></DialogHeader><form className="form-stack" onSubmit={submit}><Field label="المسار"><PathSelect paths={paths} value={form.pathId} onChange={pathId => setForm({ ...form, pathId })} /></Field><Field label="عنوان الدرس"><Input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></Field><Field label="ملخص قصير"><Textarea value={form.summary} onChange={e => setForm({ ...form, summary: e.target.value })} /></Field><DirectFileUpload mode="document" currentFileName={form.attachmentName || (form.attachmentUrl ? "ملف مرفوع حالياً" : "")} onUploaded={({ url, name }) => setForm({ ...form, attachmentUrl: url, attachmentName: name })} />{form.attachmentUrl && <Button type="button" variant="ghost" size="sm" onClick={() => setForm({ ...form, attachmentUrl: "", attachmentName: "" })}>إزالة الملف الحالي</Button>}<div className="form-row"><Field label="نوع المحتوى"><Select value={form.lessonType} onValueChange={(lessonType: "article" | "workshop" | "resource") => setForm({ ...form, lessonType })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="article">مقال</SelectItem><SelectItem value="workshop">ورشة</SelectItem><SelectItem value="resource">مورد</SelectItem></SelectContent></Select></Field><Field label="المدة بالدقائق"><Input type="number" min="1" value={form.durationMinutes} onChange={e => setForm({ ...form, durationMinutes: Number(e.target.value) })} /></Field></div><Button className="academy-button" type="submit" disabled={update.isPending || !form.pathId}>{update.isPending ? "يتم الحفظ..." : "حفظ التعديلات"}</Button></form></DialogContent></Dialog>;
}
