...
const assertApiKey = () => {
  // Local LLM servers such as Ollama do not require an API key.
  // For a remote OpenAI-compatible provider, set LLM_API_KEY in .env.
};
...
export async function listLLMModels(): Promise<ModelsResponse> {
  ...
  const url = process.env.LLM_API_URL?.trim()
    ? process.env.LLM_API_URL.replace(/\/v1\/chat\/completions\/?$/, "/v1/models")
