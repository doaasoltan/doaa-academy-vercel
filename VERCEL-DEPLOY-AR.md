...
## استكشاف الأخطاء: «You have reached your usage limits for this store»

إذا ظهر في صفحة التخزين (Vercel ← Storage ← المخزن) تنبيه:

```
You have reached your usage limits for this store using the Hobby plan.
Access resumes on <تاريخ>...
```

فهذا يعني أن **المخزن موقوف مؤقتاً** حتى التاريخ المذكور (أو حتى الترقية إلى Pro)،
ولا تعمل فيه القراءات ولا الرفع في هذه الفترة.

الحدود الشهرية للخطة المجانية (Hobby):

| المورد | الحد |
|---|---|
| التخزين | 1GB |
| انتقالات البيانات | 10GB/شهر |
| عمليات بسيطة (قراءة/كتابة) | 10,000/شهر |
| عمليات متقدمة | 2,000/شهر |

**السبب الأشيع للتجاوز:** الفيديوهات المرفوعة — كل مشاهدة لطالبة تضيف حجم الفيديو
كامله إلى الانتقالات (مثلاً فيديو 100MB يشاهده 100 طالبة = 10GB = كل الحصة!).

الحلول:

1. **الوقاية (الموصى به):** الفيديوهات على **YouTube (غير مدرج)** — مجانية وبدون
   حدود ولا تمس مخزن Blob نهائياً، وتُستخدم فقط ملفات PDF على Blob (حجمها صغير).
2. **الانتظار:** ينتهي التوقف تلقائياً في التاريخ المذكور بالتنبيه، ثم أعيدي رفع
   الملفات غير الموجودة.
3. **الترقية إلى Pro:** ترفع التوقف فوراً، لكن انتقالات Blob تُحاسب بعدها
   (~0.05$/GB)، فلا تحل مشكلة الفيديوهات الكثيفة بل تزيد كلفتها.
